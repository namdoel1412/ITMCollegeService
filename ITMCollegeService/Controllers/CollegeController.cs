...
namespace ITMCollegeService.Controllers
{
    ...
    public class CollegeController : ControllerBase
    {
        ...
        public CollegeController(IRepositoryWrapper repositoryWrapper, IMapper mapper)
        {
        ...
        {
            var sources = await _repositoryWrapper.CollegeRepo.GetDataByIdAsync(id);
            if (sources == null)
            {
                return NotFound();
            }
            else
            {
                ...
                var sourceResults = _mapper.Map<GetCollegeDTO>(sources);
        ...
        {
            var sources = await _repositoryWrapper.CollegeRepo.GetDatas();
            if (sources == null)
            {
                return NotFound();
            }
            ...
            {
                //_logger.LogInfo($"Return array Sources filted by hotelGUID");
                var sourceResults = _mapper.Map<IEnumerable<GetCollegeDTO>>(sources);
                return Ok(sourceResults);
            }
        }
        ...
        public async Task<IActionResult> UpdateSource(int id, [FromBody] UpdateCollegeDTO source)
        {
            try
            {
                ...
                var sourceEntity = await _repositoryWrapper.CollegeRepo.GetDataByIdAsync(id);
                if (sourceEntity == null)
                {
                    return NotFound();
                }
                _mapper.Map(source, sourceEntity);
                _repositoryWrapper.CollegeRepo.UpdateData(sourceEntity);
        ...
        public async Task<IActionResult> DeleteData(int id)
        {
            try
            {
                ...
                var entity = await _repositoryWrapper.CollegeRepo.GetDataByIdAsync(id);
                if (entity == null)
                {
                    return NotFound();
                }
                return Ok(await _repositoryWrapper.CollegeRepo.DeleteData(entity));
            }
            catch (Exception ex)
            {
                if (ex.InnerException != null)
